...
def is_dfc(c): return 'card_faces' in c
def not_dfc(c): return not is_dfc(c)
def fn_rarity(r): return lambda c: c['rarity'] == r


is_mythic = fn_rarity('mythic')
is_rare = fn_rarity('rare')
is_uncommon = fn_rarity('uncommon')
def is_basic(c): return 'Basic' in c['type_line']
def is_land(c): return 'Land' in c['type_line']
def is_common(c): return c['rarity'] == 'common' and not is_land(c)
...
def get_slot_probs_neo() -> tuple:
    ...
    rares = list(filter(is_rare, cards))
    mythics = list(filter(is_mythic, cards))
    ...
    common_uncommon = list(filter(is_common, cards)) + \
        list(filter(is_uncommon, cards))
    dfc_common_uncommon = list(filter(is_dfc, common_uncommon))
    ...
    uncommon_single_faced = list(filter(not_dfc, filter(is_uncommon, cards)))
    probs = np.ones(len(uncommon_single_faced)) / len(uncommon_single_faced)
    for i in range(3):
        slot_probs.append(list(zip(probs, uncommon_single_faced)))
    # Common Single Faced
    common_single_faced = list(filter(not_dfc, filter(is_common, cards)))
    ...
    # Land
    common_lands = list(filter(lambda c: is_land(
        c) and c['rarity'] == 'common', cards))
